...
const CassandraSchemaGenerator = () => {
  ...
  const [tables, setTables] = useState([
    {
      name: 'users',
      columns: [
        { name: 'user_id', type: 'uuid', isPartitionKey: true, isClusteringKey: false },
        { name: 'email', type: 'text', isPartitionKey: false, isClusteringKey: false },
        { name: 'created_at', type: 'timestamp', isPartitionKey: false, isClusteringKey: true },
        { name: 'name', type: 'text', isPartitionKey: false, isClusteringKey: false },
      ],
    },
  ])
  ...
  const generateSchema = useCallback(() => {
    ...
    try {
      ...
      schemaString += `    'class': 'SimpleStrategy',\n`
      schemaString += `    'replication_factor': 1\n`
      schemaString += `  };\n\n`
      ...
      tables.forEach(table => {
        ...
        schemaString += clusteringKeys.length ? `${clusteringKeys.join(', ')} ASC` : 'created_at ASC'
        schemaString += ');\n\n'
      })
  ...
  const addTable = () => {
    if (tables.length < MAX_TABLES) {
      setTables([...tables, {
        ...
        columns: [{ name: 'id', type: 'uuid', isPartitionKey: true, isClusteringKey: false }],
      }])
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Cassandra Schema Generator</h1>
