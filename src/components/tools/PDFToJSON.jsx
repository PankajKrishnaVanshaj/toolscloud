...
const PDFToJSON = () => {
  ...
  const convertToJSON = async () => {
    ...
    try {
      ...
      if (extractionOptions.extractText) {
        let textContent = ''
        const pageRange = extractionOptions.pageRange === 'custom' && extractionOptions.customRange
        ...
        jsonOutput.textContent = textContent.trim()
      }

      // Note: Image extraction is more complex and would require additional processing
      if (extractionOptions.extractImages) {
        jsonOutput.images = [] // Placeholder - requires additional implementation
      }
      ...
      link.href = url
      link.download = `${file.name.split('.pdf')[0]}.json`
  ...
  // Helper function to parse custom page range
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <h1 className="text-2xl font-bold mb-6 text-gray-800">PDF to JSON Converter</h1>
