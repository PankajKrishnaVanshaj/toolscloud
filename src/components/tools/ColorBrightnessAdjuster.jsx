// app/components/ColorBrightnessAdjuster.jsx
'use client';
...
const ColorBrightnessAdjuster = () => {
  ...
  const adjustBrightness = (hex, adjust) => {
    const rgb = hexToRgb(hex);
    const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);
    const newLightness = Math.max(0, Math.min(1, hsl.l + adjust / 100));
    const adjustedRgb = hslToRgb(hsl.h, hsl.s, newLightness);
    return rgbToHex(adjustedRgb.r, adjustedRgb.g, adjustedRgb.b);
  };

  useEffect(() => {
    if (/^#[0-9A-Fa-f]{6}$/.test(baseColor)) {
      const adjusted = adjustBrightness(baseColor, brightnessAdjust);
      setAdjustedColor(adjusted);
    } else {
      setAdjustedColor('');
    }
  }, [baseColor, brightnessAdjust]);
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              {adjustedColor ? (
                ...
                <p className="text-gray-500 text-sm">
                  Enter a valid HEX color (e.g., #FF6B6B) to adjust its brightness
                </p>
              )}
            </div>
          </div>
        ...
        <div className="mt-6 bg-gray-50 p-4 rounded-lg">
          <h2 className="text-lg font-semibold mb-2">About Brightness Adjustment</h2>
          <div className="text-sm text-gray-700">
            <p>Adjust the brightness (lightness) of a base color using the HSL color space:</p>
            <ul className="list-disc ml-5 mt-1">
              <li>Lightness ranges from 0% (black) to 100% (white)</li>
              <li>Adjust from -100% to +100% relative to the base lightness</li>
              <li>Hue and saturation remain constant</li>
            </ul>
            <p className="mt-1">Use the slider to modify brightness and copy the result.</p>
          </div>
        </div>
      </div>
    </div>
