...
import { CopyToClipboard } from "react-copy-to-clipboard"; // For copying to clipboard
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <input
              ...
              max="5"
              value={titleCount}
              onChange={(e) => setTitleCount(Math.max(1, Math.min(5, e.target.value)))}
              className="w-full p-2 border rounded-md focus:ring-2 focus:ring-purple-500"
            />
          </div>
        </div>
            ...
            <ul className="space-y-2">
              {titles.map((title, index) => (
                <li key={index} className="flex items-center justify-between">
                  ...
                  <CopyToClipboard text={title} onCopy={() => setCopied(index)}>
                    <button
