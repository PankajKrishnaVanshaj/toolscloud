...
const RandomBooleanGenerator = () => {
  ...
  const [format, setFormat] = useState('text') // text, number, emoji
  const [isCopied, setIsCopied] = useState(false)

  const generateBooleans = () => {
  ...
  const downloadAsText = () => {
    ...
    link.href = url
    link.download = `booleans-${Date.now()}.txt`
    link.click()
          ...
          <div className="mt-4">
            ...
            <div className="bg-gray-50 p-4 rounded-md border border-gray-200 max-h-96 overflow-auto">
              <ul className="list-decimal list-inside text-gray-800 font-mono">
                {booleans.map((bool, index) => (
                  <li key={index} className="py-1">{bool}</li>
                ))}
              </ul>
            </div>
          </div>
