...
const TextSnakeCaseConverter = () => {
  ...
  const toSnakeCase = (text) => {
    ...
    // Split by spaces, camelCase, PascalCase, kebab-case, etc.
    result = result
    ...
    result = result.filter(word => word.length > 0).join("_");
    ...
    if (options.maxLength > 0 && result.length > options.maxLength) {
      result = result.substring(0, options.maxLength).replace(/_$/, "");
    }
  ...
  const getChanges = (original, snake) => {
    ...
    if (original.match(/[A-Z]/)) {
      changes.push("Converted uppercase to lowercase");
    }
    if (original.match(/\s+|-/)) {
      changes.push("Replaced separators with underscores");
    }
        ...
        <div className="space-y-6">
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., Hello World, camelCase, kebab-case"
              maxLength={1000}
            />
                ...
                key === "maxLength" ? (
                  <div key={key} className="flex items-center space-x-2 text-sm text-gray-600">
                    ...
                    <input
                      ...
                      className="w-20 p-1 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      min="0"
                      max="200"
                    />
                  </div>
