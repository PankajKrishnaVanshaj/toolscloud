...
const ArangoDBSchemaGenerator = () => {
  ...
  const validateInput = useCallback(() => {
    ...
    if (collections.some(c => !/^[a-zA-Z][a-zA-Z0-9_-]*$/.test(c.name))) return 'Collection names must start with a letter and contain only letters, numbers, underscores, or hyphens'
    ...
    for (const collection of collections) {
      ...
      if (collection.type === 'edge' && (!collection.fields.some(f => f.name === '_from') || !collection.fields.some(f => f.name === '_to'))) {
  ...
  const generateSchema = useCallback(() => {
    ...
    setError('')
    try {
      let schemaString = `// ArangoDB Schema\n// Generated: ${new Date().toISOString()}\n\n`

      collections.forEach(collection => {
        schemaString += `// Create ${collection.type} collection\n`
        schemaString += `db._create${collection.type === 'edge' ? 'Edge' : ''}Collection("${collection.name}");\n\n`
        ...
        collection.fields.forEach(field => {
          schemaString += `      "${field.name}": { type: "${field.type}"${field.required ? ', required: true' : ''}${field.defaultValue ? `, default: ${JSON.stringify(field.defaultValue)}` : ''} },\n`
        })

        schemaString += `    },\n`
        schemaString += `    required: [${collection.fields.filter(f => f.required).map(f => `"${f.name}"`).join(', ')}],\n`
        schemaString += `    additionalProperties: true\n`
        schemaString += `  },\n`
        schemaString += `  level: "moderate",\n`
  ...
  const updateCollection = (index, key, value) => {
    ...
    if (key === 'type' && value === 'edge') {
      // Ensure _from and _to are present for edge collections
      const hasFrom = newCollections[index].fields.some(f => f.name === '_from')
      const hasTo = newCollections[index].fields.some(f => f.name === '_to')
      if (!hasFrom) newCollections[index].fields.push({ name: '_from', type: 'string', required: true, defaultValue: '' })
      if (!hasTo) newCollections[index].fields.push({ name: '_to', type: 'string', required: true, defaultValue: '' })
    }
  ...
  const removeField = (collectionIndex, fieldIndex) => {
    ...
    // Prevent removal of required edge fields
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">ArangoDB Schema Generator</h1>
            ...
            <div key={collectionIndex} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="flex flex-col gap-3 mb-4">
                ...
                {collection.type === 'edge' && (
                  <p className="text-sm text-gray-600">Edge collections require _from and _to fields</p>
                )}
              </div>
              ...
              <div className="space-y-3">
                {collection.fields.map((field, fieldIndex) => (
                  <div key={fieldIndex} className="flex flex-wrap items-center gap-3 p-3 bg-white rounded-md border border-gray-200">
                    <input
                      ...
                      className="flex-1 min-w-[150px] p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                    ...
                    <button
                      ...
                      disabled={collection.fields.length <= 1 || (collection.type === 'edge' && (field.name === '_from' || field.name === '_to'))}
                ...
                >
                  + Add Field {collections[collectionIndex].fields.length >= MAX_FIELDS && `(Max ${MAX_FIELDS})`}
                </button>
              </div>
            </div>
          ))}
