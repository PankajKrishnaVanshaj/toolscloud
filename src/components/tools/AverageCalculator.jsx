...
const AverageCalculator = () => {
  ...
  const calculateAverages = (input) => {
    // Split input by commas or spaces and filter out empty strings
      ...
      <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-lg">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2">
            <label className="w-32 text-gray-700">Numbers:</label>
            <input
              type="text"
              value={numbers}
        ...
        {result && (
          <div className="mt-6 p-4 bg-purple-50 rounded-lg">
            ...
            <div className="mt-2 space-y-2">
              <p className="text-center">Mean: {result.mean}</p>
              <p className="text-center">Median: {result.median}</p>
              <p className="text-center">Mode: {result.mode}</p>
              ...
              {showDetails && (
                <div className="text-sm space-y-2">
                  ...
                  <ul className="list-disc list-inside">
                    ...
                    <li>Mean = (Sum of numbers) / Count = ({result.numbers.reduce((sum, num) => sum + num, 0)}) / {result.numbers.length} = {result.mean}</li>
                    <li>Sorted Numbers: {result.sortedNumbers.join(', ')}</li>
                    <li>Median = {result.numbers.length % 2 === 0 
                      ? `Average of middle numbers (${result.sortedNumbers[Math.floor(result.numbers.length / 2) - 1]} + ${result.sortedNumbers[Math.floor(result.numbers.length / 2)]}) / 2 = ${result.median}`
                      : `Middle number = ${result.median}`}</li>
                    <li>Mode = Most frequent number(s): {result.mode === 'No mode' ? 'All numbers appear once' : result.mode}</li>
                  </ul>
                </div>
              )}
            </div>
