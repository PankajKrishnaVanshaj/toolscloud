...
const CouchDBSchemaGenerator = () => {
  const [documents, setDocuments] = useState([
    {
      name: 'user',
      fields: [
        { name: '_id', type: 'string', required: true, defaultValue: '' },
        { name: 'type', type: 'string', required: true, defaultValue: 'user' },
        { name: 'username', type: 'string', required: true, defaultValue: '' },
        { name: 'email', type: 'string', required: false, defaultValue: '' },
        { name: 'created_at', type: 'timestamp', required: true, defaultValue: 'new Date().toISOString()' },
      ],
      views: [
        {
          name: 'by_username',
  ...
  const validateInput = useCallback(() => {
    ...
    if (documents.some(d => !/^[a-z][a-z0-9_]*$/.test(d.name))) return 'Document names must be lowercase with letters, numbers, or underscores'
    ...
    for (const doc of documents) {
      ...
      if (new Set(doc.fields.map(f => f.name)).size !== doc.fields.length) return `Field names in "${doc.name}" must be unique`
      ...
      documents.forEach(doc => {
        ...
        doc.fields.forEach(field => {
          if (field.name !== '_id' && field.name !== 'type') {
            let value
            switch (field.type) {
              case 'string': value = field.defaultValue || ''; break
              case 'number': value = field.defaultValue ? Number(field.defaultValue) : 0; break
              case 'boolean': value = field.defaultValue === 'true'; break
              case 'array': value = field.defaultValue ? JSON.parse(field.defaultValue) : []; break
              case 'object': value = field.defaultValue ? JSON.parse(field.defaultValue) : {}; break
              case 'timestamp': value = field.defaultValue || new Date().toISOString(); break
            }
  ...
  const addDocument = () => {
    if (documents.length < MAX_DOCUMENTS) {
      setDocuments([...documents, {
        ...
        fields: [{ name: '_id', type: 'string', required: true, defaultValue: '' }],
        views: [],
      }])
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">CouchDB Schema Generator</h1>
                ...
                {doc.views.map((view, viewIndex) => (
                  <div key={viewIndex} className="space-y-2 p-3 bg-white rounded-md border border-gray-200">
                    ...
                    <textarea
                      ...
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 min-h-[100px]"
                    />
                    <textarea
                      value={view.reduce}
                      onChange={(e) => updateView(docIndex, viewIndex, 'reduce', e.target.value)}
                      placeholder="Reduce function (optional)"
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 min-h-[60px]"
                    />
                  </div>
                ))}
                <button
