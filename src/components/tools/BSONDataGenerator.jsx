...
import { serialize } from 'bson'; // Named import instead of top-level BSON
...
const BSONDataGenerator = () => {
  ...
  const generateRandomData = useCallback((type, required) => {
    ...
    switch (type) {
      ...
      case 'boolean':
        ...
        break;
      case 'date':
        value = new Date(timestamp - Math.random() * 31536000000);
        break;
      case 'array':
      ...
      case 'object':
        ...
        break;
      case 'binary':
        value = Buffer.from(Array.from({ length: 16 }, 
  ...
  const generateBSONData = useCallback(() => {
    ...
    try {
      ...
      const bsonBuffer = serialize(dataStructure); // Use named import
  ...
  // Rest of your code (validateFields, addField, updateField, etc.) remains the same...
