...
const TextKebabCaseConverter = () => {
  ...
  const toKebabCase = (text) => {
    ...
    // Split by spaces, camelCase, PascalCase, snake_case, etc.
    result = result
    ...
    if (options.maxLength > 0 && result.length > options.maxLength) {
      result = result.substring(0, options.maxLength).replace(/-$/, "");
    }
  ...
  const getChanges = (original, kebab) => {
    ...
    if (original.match(/\s+|_|-/)) {
      changes.push("Replaced separators with hyphens");
    }
        ...
        <div className="space-y-6">
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., Hello World, camelCase, snake_case"
              maxLength={1000}
            />
