...
const XMLDataGenerator = () => {
  ...
  const generateXML = () => {
    if (fields.length === 0) {
      setXmlData('Please add at least one field!')
      return
    }
    ...
    for (let i = 0; i < Math.min(count, 100); i++) {
      ...
      fields.forEach(field => {
        xml += `    <${field.name}>${generateRandomData(field.type)}</${field.name}>\n`
      })
      xml += `  </${itemElement}>\n`
    }
  ...
  const downloadAsXML = () => {
    ...
    link.href = url
    link.download = `data-${Date.now()}.xml`
    link.click()
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            ...
            <input
              type="text"
              value={rootElement}
              onChange={(e) => setRootElement(e.target.value || 'items')}
              className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            ...
            <input
              type="text"
              value={itemElement}
              onChange={(e) => setItemElement(e.target.value || 'item')}
              className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
