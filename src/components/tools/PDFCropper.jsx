...
const PDFCropper = () => {
  ...
  const cropPDF = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        const [x, y, width, height] = cropSettings.applyToAll || i === currentPage
          ? [
              cropArea.x + cropSettings.margins.left,
              cropArea.y + cropSettings.margins.top,
              cropArea.width || page.view[2] - (cropSettings.margins.left + cropSettings.margins.right),
              cropArea.height || page.view[3] - (cropSettings.margins.top + cropSettings.margins.bottom),
            ]
          : [0, 0, page.view[2], page.view[3]];
        ...
        await page.render({
          canvasContext: ctx,
          viewport: page.getViewport({ scale: cropSettings.quality }),
          transform: [1, 0, 0, 1, -x, -y],
        }).promise;

        const imgData = canvas.toDataURL("image/png");
        await newPdf.addImage(imgData, "PNG", 0, 0, width, height);
        await newPdf.addPage();
      }
  ...
  const reset = () => {
    ...
    setCurrentPage(1);
    setCropArea({ x: 0, y: 0, width: 0, height: 0 });
    setCropSettings({ applyToAll: false, units: "px", margins: { top: 0, right: 0, bottom: 0, left: 0 }, quality: 1 });
